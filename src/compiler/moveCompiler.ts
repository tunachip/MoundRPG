...
export function parseMoveFile(source: string, sourcePath?: string): ParsedMoveFile {
	...
	return {
		move: {
			...
			canChain: metadata.canChain ?? false,
			canBeChainedInto: metadata.canBeChainedInto ?? false,
			operations,
		},
		sourcePath,
	};
...
function getTemplateImportPath(outputPath: string): string {
	const templatePath = path.resolve(path.dirname(outputPath), '../engine/templates/move.ts');
	const repoTemplatePath = path.resolve(path.dirname(outputPath), relativeEngineTemplatePath(outputPath));
...
function splitSections(source: string): Record<string, RawLine[]> {
...
function parseNotes(lines: RawLine[]): string {
	return lines.map((line) => line.text).join(' ').trim();
}

function parseMetadata(lines: RawLine[]): Partial<MoveTemplate> {
...
function assignLifecycleBlock(
	...
	if (conditionalBlock.conditions.length > 0 || conditionalBlock.operations.length > 0) {
		operations[lifecycleKey] = conditionalBlock;
	}
}

function parseConditionalOperationsBlock(lines: RawLine[]): ConditionalOperations {
	...
	return {
		conditions: conditionsHeading === -1
			? []
			: parseConditionList(sliceHeadingBlock(lines, conditionsHeading)),
		operations: operationsHeading === -1
			? []
			: parseOperationList(sliceHeadingBlock(lines, operationsHeading)),
	};
}
...
function parseOperationNode(name: string, lines: RawLine[], parentIndent: number): Operation {
	...
	while (index < lines.length) {
		...
		if (isOperationHeading(heading)) {
			const { blockLines, nextIndex } = collectPeerBlock(lines, index, parentIndent, [
				'OPERATIONS',
				'CONDITIONS',
				'IF',
				'THEN',
...
function parseIfBlock(lines: RawLine[]): Record<string, unknown> {
	...
	while (index < lines.length) {
		...
		if (numbered) {
			...
			while (index < lines.length) {
				...
				if (pair && normalizeLabel(pair.key) === 'EXPECTS' && lines[index].indent === startIndent) {
					break;
				}
				index += 1;
			}
...
function parseConditionList(lines: RawLine[]): Array<Condition> {
	...
	while (index < lines.length) {
		...
		if (numbered) {
			...
			if (index < lines.length) {
				...
				if (expectsPair && normalizeLabel(expectsPair.key) === 'EXPECTS') {
...
function parseConditionNode(name: string, lines: RawLine[]): Condition {
	const args: Record<string, unknown> = {};
	let expects: unknown;
...
function findHeadingIndex(lines: RawLine[], heading: string): number {
	return lines.findIndex((line) => normalizeLabel(line.text) === heading);
}

function sliceHeadingBlock(lines: RawLine[], headingIndex: number): RawLine[] {
	...
	while (endIndex < lines.length) {
		...
		if (lines[endIndex].indent <= headingLine.indent && peerHeadings.has(heading)) {
			break;
		}
		endIndex += 1;
	}
...
function collectPeerBlock(
	...
	while (endIndex < lines.length) {
		...
		if (lines[endIndex].indent === startLine.indent && peerHeadings.includes(heading)) {
			break;
		}
		endIndex += 1;
	}
...
function isOperationHeading(label: string): boolean {
	return ['OPERATIONS', 'CONDITIONS', 'IF', 'THEN', 'ELSE'].includes(label);
}

function parseNumberedItem(text: string): { name: string } | null {
...
function toArgKey(key: string): string {
	const normalized = normalizeLabel(key).toLowerCase();
	return normalized.replace(/_([a-z])/g, (_, char: string) => char.toUpperCase());
}
