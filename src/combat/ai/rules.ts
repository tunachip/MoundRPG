...
export const TemperamentRules: Record<TemperamentTag, TemperamentRule> = {
	'utilitarian': ({ move }) =>
		move.type === 'utility' ? 2 : -0.5,
	'aggressive': ({ move }) =>
		move.type === 'attack' ? 2.5 : -0.4,
	'health-conscious': ({ intent, move }) =>
		hasSelfSustainOperation(resolveOperations(move, intent)) ? 2 : 0,
	...
	'tactical': (ctx) =>
		...
		+ scoreDefensePressure(
			ctx.combat,
			ctx.caster,
			ctx.move.element
		) * 0.5,
	'stately': ({ intent, move }) =>
		hasSelfApplyStatus(resolveOperations(move, intent)) ? 2 : 0,
	'defensive': (ctx) =>
		scoreDefensePressure(
			ctx.combat,
			ctx.caster,
	...
	'frugal': ({ chainIndex }) =>
		chainIndex > 0 ? -1.5 : 0.5,
	'spendthrift': ({ chainIndex }) =>
		chainIndex > 0 ? 1.5 : 0.2,
};
...
function scoreDefensePressure (
	...
	let score = 0;
	const enemyElements = findEnemyActiveAttackElements(combat, caster.index);
	for (const enemyElement of enemyElements) {
		const rules = getDamageRules(enemyElement);
		switch (true) {
			case rules.absorbs.includes(element):
				score += 2;
			case rules.blocks.includes(element):
				score += 1.5;
			case rules.resists.includes(element):
				score += 1;
			case rules.weakTo.includes(element):
				score -= 0.5;
		}
	}
	return score;
...
function resolveOperations (
	...
	return intent === 'cast' ? operations.fromActive : operations.fromBanked;
}
...
function hasSelfSustainOperation (
	...
	return operations.some((operation) => {
		if (!['heal', 'applyEnergy', 'raiseMaxHp', 'raiseMaxEnergy']
